        ...
        for _cycle in range(context.max_cycles):
            ...
            for proposal in proposals:
                ...
                if context.mode.value in {"dry_run", "replay", "canary_shadow"}:
                    ...
                    continue
                if needs_approval:
                    approval_args = {"workspace_id": str(request.workspace_id), "run_id": str(request.run_id), "action_id": planned["action_id"], "action_revision": planned["revision"], "request_hash": planned["request_hash"], "credential_ref": str(proposal.get("credential_binding_id")) if proposal.get("credential_binding_id") else None, "expires_at": proposal.get("approval_expires_at")}
                    if workflow.patched("approval-request-display-v1"):
                        approval_args.update({"risk_level": policy.get("risk_level", "unknown"), "policy_ref": policy.get("policy_version"), "policy_explanation": {"reason": policy.get("reason", "Approval required")}})
                    approval_request = await workflow.execute_activity(create_approval_request, approval_args, start_to_close_timeout=timedelta(seconds=30), retry_policy=RetryPolicy(maximum_attempts=3))
