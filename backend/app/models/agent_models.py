...
class Agent(Base):
    ...
    __table_args__ = (Index("uq_agents_thread_handle_ci", "thread_id", text("lower(handle)"), unique=True), Index("uq_agents_thread_moderator", "thread_id", unique=True, postgresql_where=text("is_moderator")), Index("uq_agents_system_thread", "thread_id", unique=True, postgresql_where=text("is_system")), CheckConstraint("concurrency_limit > 0"), CheckConstraint("queue_limit >= 0"), CheckConstraint("status IN ('draft','active','paused','archived')"), CheckConstraint("execution_mode IN ('observe','recommend','act')"), CheckConstraint("NOT is_system OR is_moderator", name="ck_agents_system_is_moderator"))
