...
def main() -> None:
    ...
    parser.add_argument("--voice-reachy-backend", choices=("sdk", "sdk-release", "alsa", "alsa-release", "pulse", "pulse-release"), default=os.environ.get("REACHY_VOICE_REACHY_BACKEND", "alsa-release"), help="Capture backend when --voice-source=reachy. alsa uses reachymini_audio_src directly; sdk uses ReachyMini.media; *-release asks the daemon to release media before capture.")
    ...
    parser.add_argument("--voice-reachy-silence-fallback-windows", type=int, default=int(os.environ.get("REACHY_VOICE_REACHY_SILENCE_FALLBACK_WINDOWS", "0")), help="Silent Reachy microphone windows before falling back to host microphone. Defaults to 0, which disables host fallback.")
