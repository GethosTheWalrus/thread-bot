...
- **`web_fetch` pagination** — Returns a paginated window of page content (default 5,000 chars from offset 0). Accepts `start_index` and `max_chars` parameters. Response includes a metadata header with total page length and instructions to continue reading. This prevents large pages from overwhelming the LLM context while still allowing full page access via multiple calls.
