...
![ThreadBot Header](https://raw.githubusercontent.com/google-deepmind/antigravity/main/assets/threadbot_banner.png) *(Placeholder for banner)*

## ✨ Key Features

- **🧵 Thread-Based Conversations**: Organize your chats into threads with automatic title generation.
- **🛠️ MCP Tool Support**: Integrate with any MCP-compatible tool server via Docker sidecars.
- **🧠 Advanced Memory**: 
...
ThreadBot is fully containerized and easy to run locally using Docker Compose.

### Prerequisites
- Docker & Docker Compose
- An OpenAI-compatible LLM API (e.g., [Ollama](https://ollama.com/), OpenAI, or Anthropic)

### Launch
```bash
git clone git@git.home:mike/thread-bot.git
cd thread-bot
